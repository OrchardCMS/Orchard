...
namespace Orchard.Data.Migration.Interpreters {
    public class MySqlCommandInterpreter : ICommandInterpreter<AlterColumnCommand> {
        ...
        public string[] CreateStatements(AlterColumnCommand command) {
            ...
            {
                builder.Append(DefaultDataMigrationInterpreter.GetTypeName(_dialectLazy.Value, command.DbType,
                    command.Length, command.Precision, command.Scale));
            }
            else
            {
            ...
            {
                builder2.Append(" set default ")
                    .Append(DefaultDataMigrationInterpreter.ConvertToSqlValue(command.Default))
                    .Append(" ");
            }

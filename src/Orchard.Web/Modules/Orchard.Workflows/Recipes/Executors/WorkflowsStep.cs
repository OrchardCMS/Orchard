        ...
        public override void Execute(RecipeExecutionContext context)
        {
            foreach (var workflowDefinitionElement in context.RecipeStep.Step.Elements())
            {
                ...
                try
                {
                    ...
                    workflowDefinition.Enabled = bool.Parse(workflowDefinitionElement.Attribute("Enabled").Value);

                    foreach (var activityElement in activitiesElement.Elements())
                    {
                        var localId = int.Parse(activityElement.Attribute("Id").Value);
                        ...
                        {
                            Name = activityName,
                            Start = bool.Parse(activityElement.Attribute("Start").Value),
                            X = int.Parse(activityElement.Attribute("X").Value),
                            Y = int.Parse(activityElement.Attribute("Y").Value),
                    ...
                    {
                        var sourceActivityId = int.Parse(transitionElement.Attribute("SourceActivityId").Value);
                        var sourceEndpoint = transitionElement.Attribute("SourceEndpoint").Value;
                        var destinationActivityId = int.Parse(transitionElement.Attribute("DestinationActivityId").Value);

...
(function ($) { function Calendars() { this.regionalOptions = []; this.regionalOptions[''] = { invalidCalendar: 'Calendar {0} not found', invalidDate: 'Invalid {0} date', invalidMonth: 'Invalid {0} month', invalidYear: 'Invalid {0} year', differentCalendars: 'Cannot mix {0} and {1} dates' }; this.local = this.regionalOptions['']; this.calendars = {}; this._localCals = {}; } $.extend(Calendars.prototype, { instance: function (a, b) { a = (a || 'gregorian').toLowerCase(); b = b || ''; var c = this._localCals[a + '-' + b]; if (!c && this.calendars[a]) {
    ...
    throw ($.calendars.local.invalidDate || $.calendars.regionalOptions[''].invalidDate).replace(/\{0\}/, this._calendar.local.name);
        ...
        var d = c.day();
        var i = function (a) { while (m < a.minMonth) {
            y--;
            m += a.monthsInYear(y);
        } var b = a.monthsInYear(y); while (m > b - 1 + a.minMonth) {
            y++;
            m -= b;
            b = a.monthsInYear(y);
        } };
        ...
        this._validateLevel--;
        throw e;
    } }, _correctAdd: function (a, b, c, d) { if (!this.hasYearZero && (d === 'y' || d === 'm')) {
        if (b[0] === 0 || (a.year() > 0) !== (b[0] > 0)) {
            var e = { y: [1, 1, 'y'], m: [1, this.monthsInYear(-1), 'm'], w: [this.daysInWeek(), this.daysInYear(-1), 'd'], d: [1, this.daysInYear(-1), 'd'] }[d];
            var f = (c < 0 ? -1 : +1);
            b = this._add(a, c * e[0] + f * e[1], e[2]);
        }
    } return a.date(b[0], b[1], b[2]); }, set: function (a, b, c) { this._validate(a, this.minMonth, this.minDay, $.calendars.local.invalidDate || $.calendars.regionalOptions[''].invalidDate); var y = (c === 'y' ? b : a.year()); var m = (c === 'm' ? b : a.month()); var d = (c === 'd' ? b : a.day()); if (c === 'y' || c === 'm') {
        d = Math.min(d, this.daysInMonth(y, m));
    } return a.date(y, m, d); }, isValid: function (a, b, c) { this._validateLevel++; var d = (this.hasYearZero || a !== 0); if (d) {
    ...
    } this._validateLevel--; return d; }, toJSDate: function (a, b, c) { var d = this._validate(a, b, c, $.calendars.local.invalidDate || $.calendars.regionalOptions[''].invalidDate); return $.calendars.instance().fromJD(this.toJD(d)).toJSDate(); }, fromJSDate: function (a) { return this.fromJD($.calendars.instance().fromJSDate(a).toJD()); }, _validate: function (a, b, c, d) { if (a.year) {
        if (this._validateLevel === 0 && this.name !== a.calendar().name) {
            throw ($.calendars.local.differentCalendars || $.calendars.regionalOptions[''].differentCalendars).replace(/\{0\}/, this.local.name).replace(/\{1\}/, a.calendar().local.name);
        }
        return a;
    } try {
        this._validateLevel++;

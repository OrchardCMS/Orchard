...
(function ($) {
    ...
    return newTime;
        ...
        _postAttach: function (elem, inst) {
            elem.on('focus.' + inst.name, this._doFocus).
                ...
                on('paste.' + inst.name, function (event) {
        ...
        _enableDisable: function (elem, disable) {
            ...
            this._disabledInputs = $.map(this._disabledInputs, function (value) { return (value === elem ? null : value); }); // Delete entry
            if (disable) {
        ...
        _preDestroy: function (elem, inst) {
            ...
            this._disabledInputs = $.map(this._disabledInputs, function (value) { return (value === elem[0] ? null : value); }); // Delete entry
 ...
 $(selector).timeEntry('setTime', null) */
        ...
        _handleSpinner: function (event) {
            ...
            if (region >= 3 && inst.options.spinnerRepeat[0]) {
                plugin._timer = setTimeout(function () { plugin._repeatSpinner(inst, region); }, inst.options.spinnerRepeat[0]);
        ...
        _repeatSpinner: function (inst, region) {
            ...
            this._timer = setTimeout(function () { plugin._repeatSpinner(inst, region); }, inst.options.spinnerRepeat[1]);
        },
        ...
        _parseTime: function (inst, event) {
            ...
            if (typeof inst.options.initialField === 'number') {
                inst._field = Math.max(0, Math.min(Math.max(1, inst._secondField, inst._ampmField), inst.options.initialField));
                postProcess();
            }
            else {
                setTimeout(function () {
        ...
        _adjustField: function (inst, offset) {
            ...
            if (inst.options.unlimitedHours) {
                ...
                this._setTime(inst, new Date(0, 0, 0, inst._selectedHour + (inst._field === 0 ? offset * inst.options.timeSteps[0] : 0) +
                    (inst._field === inst._ampmField ? offset * 12 : 0), inst._selectedMinute + (inst._field === 1 ? offset * inst.options.timeSteps[1] : 0), inst._selectedSecond + (inst._field === inst._secondField ? offset * inst.options.timeSteps[2] : 0)));
            }
        },

...
namespace Orchard.CustomForms.Drivers
{
    public class CustomFormPartDriver : ContentPartDriver<CustomFormPart>
    {
        ...
        protected override void Importing(CustomFormPart part, ImportContentContext context)
        {
            ...
            context.ImportAttribute(part.PartDefinition.Name, "UseContentTypePermissions", x => part.Record.UseContentTypePermissions = bool.Parse(x));
            context.ImportAttribute(part.PartDefinition.Name, "SaveContentItem", x => part.Record.SaveContentItem = bool.Parse(x));
            context.ImportAttribute(part.PartDefinition.Name, "SavePublishContentItem", x => part.Record.SavePublishContentItem = bool.Parse(x));
            context.ImportAttribute(part.PartDefinition.Name, "CustomMessage", x => part.Record.CustomMessage = bool.Parse(x));
            context.ImportAttribute(part.PartDefinition.Name, "Message", x => part.Record.Message = x);
            context.ImportAttribute(part.PartDefinition.Name, "Redirect", x => part.Record.Redirect = bool.Parse(x));

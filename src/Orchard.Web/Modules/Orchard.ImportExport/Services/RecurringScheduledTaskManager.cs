...
namespace Orchard.ImportExport.Services {
    ...
    public class RecurringScheduledTaskManager : IRecurringScheduledTaskManager {
        ...
        public RecurringScheduledTaskManager(
            IClock clock,
        ...
        public ScheduledTaskRunHistory GetLastTaskRun(int id, RunStatus? runStatus) {
            return _runHistoryRepository.Table.OrderByDescending(t => t.Id)
                .FirstOrDefault(t => 
                    t.ContentItemRecord.Id == id
                    && (!runStatus.HasValue || t.RunStatus == runStatus));
        }

        public IScheduledTask GetNextScheduledTask(int id) {
            return _scheduledTaskManager
                .GetTasks(_contentManager.Get(id))
                .OrderByDescending(t => t.ScheduledUtc)
                .FirstOrDefault();
        }

        public IList<ScheduledTaskRunHistory> GetHistory(int? id, int limit = 20) {
            return _runHistoryRepository
                .Table
        ...
        public ScheduledTaskRunHistory SetTaskStarted(RecurringTaskPart taskPart, string executionId) {
            ...
            var runHistory = new ScheduledTaskRunHistory {
                ContentItemRecord = taskPart != null
                    ? taskPart.Record.ContentItemRecord
                    : null,
        ...
        public void ClearHistory(double daysToRetain) {
            ...
            for (var i = forDeletion.Count - 1; i >= 0; i--) {
                _runHistoryRepository.Delete(forDeletion[i]);
            }
        }

        private DateTime CalculateNextRunUtc(RecurringTaskPart part) {
            return part.RepeatFrequencyInMinutes > 0 
                ? _clock.UtcNow.AddMinutes(part.RepeatFrequencyInMinutes) 
                : _clock.UtcNow;
        }

        private void ClearExistingTasks(ContentPart part) {
            _scheduledTaskManager.DeleteTasks(
                part.ContentItem, 
                t => t.TaskType == part.Settings["TaskType"]);
        }
